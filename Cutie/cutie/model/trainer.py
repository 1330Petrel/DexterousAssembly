...
class Trainer:
    def __init__(self, cfg: DictConfig, stage_cfg: DictConfig, log: TensorboardLogger, run_path):
        ...
        if self.use_amp:
            self.scaler = torch.amp.grad_scaler.GradScaler("cuda", init_scale=8192)
    ...
    def load_checkpoint(self, path):
        ...
        checkpoint = torch.load(path, map_location={'cuda:0': map_location}, weights_only=False)
    ...
    def load_weights(self, path):
        ...
        map_location = 'cuda:%d' % self.local_rank
        src_dict = torch.load(path, map_location={'cuda:0': map_location}, weights_only=False)
